...
独立的 AstrBot `qq_official_v2` 平台适配器，提供官方协议接入、媒体与基础收发、流式、指令键盘和 OneBot v11 风格 OpenID 子集。当前 **v0.4.0 为预览版**；外链转存与Base64分片已通过限定群出站复测，完整接入、回调及Windows部署仍待验。
...
未开放：OneBot HTTP/WS监听、完整好友/群枚举、全员禁言、群文件系统、合并转发及缺完整记录的 `get_msg`。`/v2/menu` 自定义菜单仍独立于已实现的 `/v2/panels`（含C2C all/specific），不会接管人工菜单。管理资料与互动不会建立聊天身份；不支持的事件保留，不意外触发LLM。

| 扩展 | 实现与条件 |
|---|---|
| 群/C2C媒体 | PNG/JPEG、SILK、MP4、文件；普通URL上传及prepare→PUT→finish→files；整链预检，不静默丢caption或拆多条 |
